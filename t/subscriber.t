...
while (my $recv = K::Raw::k($handle)) {
    ...
    my $tn = ($t*$NANOS_PER_SEC) - $EPOCH_OFFSET_NANO;
    my $k_latency = ($tn - $values->{time}->[0]) / 1000;

    # use Devel::Peek;
    # Dump($values->{time}->[0]);

    say "$$> $table: $values->{time}->[0] | $tn (latency: $k_latency microsecs)";
